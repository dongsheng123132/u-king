...
升级流程：先把目标上游 release commit 导入并在 fork 中标记，之后才可将 fork 作为新发布包的可追溯来源；从该精确提交重新计算 lock 中两个哈希并验证构建；生成新的 bundle ID、归档和完整性清单；最后在全部验证通过后，单独更新发布 catalog。绝不复用、覆盖或冒充既有 catalog 条目；`.3` 在 `.2` 的本地项目桥接基础上，新增了仅 U-King 宿主模式下的托管供应商提示。
