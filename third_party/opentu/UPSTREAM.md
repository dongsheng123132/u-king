...
当前构建目标 `.7` 恢复 OpenTu 原生 API Key 设置与生成操作，默认使用虾盘云兼容接口。U-King 可向当前画布自动填入客户的虾盘云 Key；未收到 Key 时仍可手动填写。旧版禁用原生生成、隐藏供应商设置的两个补丁已撤回，外层重复生成入口也已移除。项目持久化沿用原有桥接；保存时通过 OpenTu 现有缓存服务把本地缓存图片内联进项目，避免重启换端口后图片丢失。无法读取图片时明确报告保存失败。桥接同时跟踪全局和当前画布版本：其他项目变化可有限重读并重试，同一画布的真实冲突仍拒绝覆盖；确认回包丢失时先核对已落盘内容。上游固定版本保持不变。
