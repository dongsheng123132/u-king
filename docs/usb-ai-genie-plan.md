...
```text
U-King.exe
U-King-ToolDisk-Windows-x64.zip
U-King-Setup.exe
```
...
| 上游版本锚点 | tag `0.3.1`，取证提交 `2cf030d2` |
| 压缩包 | 22,385,616 字节 |
| 解压内容 | 62,157,806 字节，约 59.3MB |
| 冷启动三次 | 120.3452ms、107.4152ms、92.3664ms |
...
| U 盘随机读 | 38.10MB/s |
| U 盘顺序读 | 106.34MB/s |
| 已知兼容问题 | `deepseek-v4-flash` thinking 模式多轮 tool-call 可触发 `reasoning_content` 相关 HTTP 400 |
...
```json
{
  ...
  "asset_url": "<经核验的上游 release asset>",
  "sha256": "<发布前实取并复核>",
...
禁止在计划阶段猜写 asset URL 或 SHA-256；P0 使用已取证压缩包，P1 发布前从上游正式 release 产物实取并固定。
...
- `check-picoclaw-bundle.mjs` 校验版本、hash、文件清单、LICENSE、NOTICE、启动器和禁止项。
...
rem PicoClaw 的交互历史使用 os.TempDir；TEMP/TMP 必须跟着工具盘，
rem 避免提示词历史写进宿主机 %TEMP%。
...
- 暂不开放 credential 加密、DeltaChat、企业微信频道；
...
P0 可用报告已验证的网关地址复现手工链路；P1 不再在 `usb_genie.rs` 复制网关常量，而是从 U-King 现有 provider 模板读取当前官方端点，避免多份事实漂移。
...
4. 运行 `onboard`。
5. 使用一次性测试 key，将网关配置落入 `config.json` 和 `.security.yml`。
...
- 报告和脚本输出中不存在完整 key。
- 中文及空格目录成功。
- thinking 400 被正确识别为模型多轮兼容问题，不显示成“key 无效”。
...
Rust 测试不得少于当前 477 个既有用例，并应增加 usb_genie 专项用例。
...
该问题必须作为已知兼容缺陷处理，不能归因于网络或 API key。
...
1. P0 先验证 PicoClaw 0.3.1 是否提供可靠的关闭 thinking 配置；没有证据前不得假设存在。
...
- 生成 zip 和 SHA-256；
- 运行 Windows 工具盘冒烟；
- 上传 `U-King-ToolDisk-Windows-x64.zip`。
...
| 9月7日～12日 | P1 Rust 动作核心和前端板块 | 12～14小时 |
