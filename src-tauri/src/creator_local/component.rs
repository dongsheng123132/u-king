...
mod tests {
    ...
    fn embedded_catalogue_offers_only_the_built_in_component() {
        crate::testsandbox::with_sandbox("opentu-component-catalogue", &[], |_| {
            ...
            assert_eq!(offer.bundle_id.as_deref(), Some("opentu-1.1.6-uking.4"));
            assert_eq!(offer.archive_bytes, Some(18_561_152));
            let (manifest, _) = trusted_offer().unwrap().unwrap();
            assert_eq!(manifest.bundle_id, "opentu-1.1.6-uking.4");
            assert_eq!(manifest.archive_bytes, 18_561_152);
