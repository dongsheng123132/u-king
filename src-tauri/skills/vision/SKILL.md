...
很多模型（尤其 **DeepSeek** 聊天系列）**只会处理文字，收不了图片**。本技能给它当「眼睛」：把图片交给一个会看图的模型读成文字，再交回你（DeepSeek）继续干活。**你的对话模型不用换、省钱路由不动**，只在遇到图片时调一下本技能（技能内部默认走的视觉模型是 `deepseek-v4-flash`，见下「为什么默认用 deepseek-v4-flash」——它跟你聊天用的 DeepSeek 不是同一路，聊天那路依旧看不了图）。
...
**别以为默认模型不需要裁**：2026-09-24 前的旧默认 `qwen3.7-flash` 在 2400×2908 长截图上确实
不用裁（4/4）；现默认 `deepseek-v4-flash` 在同类长截图上是 0/4——正是下一节「长图分块」自动
触发的原因，不用你手动 `--region`。
...
结论（2026-09-16 时点）：`uking-vision` 保留、继续分发，DeepSeek 全系仍在闸门里被拦（`TEXT_ONLY`
正则和拦截行为没变）——变的只是拦截理由，`see-image.mjs` 里的 `whyBlocked(id)` 现在按模型给准确
说法，不再统一说「全系纯文本」。

**2026-09-24 更新**：产品决定权衡「体验一致（DeepSeek 自己识图，不用换供应商）」与「准确率」后，
把 `see-image.mjs` 的默认模型改回 **`deepseek-v4-flash`**（上表 62% 那档，长截图弱点靠上面「长图
...
| `--model <id>` | 换模型。默认走三棒链 `deepseek-v4-flash` → `qwen3.7-plus` → `kimi-k3`（走了替补会在 `fallback_from` 里说明；显式指定 `--model` 时**不**自动换）。想要更准可显式 `--model qwen3.7-flash`（原默认，95% vs 现默认 62%）。整条链有 240s 总预算，不会假死。**纯文本模型会被当场拒绝**，见下 |
...
| 棒次 | 模型 | 挡什么 | 实测 |
|---|---|---|---|
| 主力 | `deepseek-v4-flash` | — | 合计 62%，长截图分块前 0/4（已靠自动「长图分块」缓解），见上文「为什么默认用 deepseek-v4-flash」 |
| ① | `qwen3.7-plus` | 主力抽风 / 主力那条路由整条断 | 95%，快（4~6s）。2026-09-24 改默认后**跟主力不同厂商路由**（阿里百炼 vs DeepSeek），主力断时这里大概率还在，且比主力更准 |
| ② | `kimi-k3` | 再挡一层路由断 | 裸名=月之暗面直连，账号/端点跟阿里、DeepSeek 都无关。**慢**：长截图中位 61s、峰值 129s，泛问命中 1~4/4 不稳；`--ask` 稳定 4/4 |
...
## 为什么默认用 deepseek-v4-flash（2026-09-24 起；有跑道，可自己复跑）

**当前默认**：`deepseek-v4-flash`。2026-09-24 产品决定——主力对话模型本来就是 DeepSeek，识图也走
自家模型，体验更顺、不用来回切供应商；代价是准确率不如下面表里的 `qwen3.7-flash`（62% vs
95%，尤其长截图 4 次全 0，已靠上面「长图分块」缓解）。想要更准可 `--model qwen3.7-flash` 显式切回。
详见上面「DeepSeek 自带视觉够不够用（2026-09-16）」一节的对照数据。

**下表是 2026-08-16 时点、`qwen3.7-flash` 还是默认时留下的历史选型数据**（`node scripts/bench.mjs
--cases bench/cases.json --repeat 3` —— 三类**全合成夹具**，零隐私，`bench/gen-fixtures.mjs` 生成，
needles 是图里客观存在、必须被读出来的字符串，不做模糊匹配）：

| 模型 | 证照字段 | 大图小字(2400px) | 长截图(2400×2908) | 耗时中位 |
|---|---|---|---|---|
| **qwen3.7-flash**（现为可选覆盖） | 8/8 | 7/7 | **4/4** | 13.1s |
...
| MiniMax-M3（曾经的旧默认） | 6.7/8 | **泛问 0/7** | 未测 | 6.6s |

三条结论（历史，当时据此选了 qwen3.7-flash 当默认）：

1. **长截图是分水岭**：只有 qwen3.7-flash 扛住（4/4），其余从 44% 一路崩到 13%。
2. **MiniMax-M3 的失败模式是「编」不是「漏」** —— 泛问那张宽截图三遍全 0/7，还编出不存在的按钮、账号和公司名。confidently wrong 比读不出来危险得多，这是当时换默认的直接原因。
