...
- 修复内嵌终端的滚动、粘贴和 ANSI 颜色显示；对话与终端工作台入口更清楚。
- 技术支持改为网页入口；本地模型入口收敛为 Ollama 与 llama.cpp。
- 本地引擎检测增加查询时限，服务响应缓慢时及时返回，并避免重复查询模型列表。
- AI 体检结果可一键复制，并自动交给对话工作台继续给出优化建议。
- 多个供应商测速同时进行时，请求正文不再互相串位。
- 创作画布入口保留待后续上线，本次不开放该功能。

**开发者**

- 供应商请求的临时 JSON 文件改用进程内唯一序列，避免并发 curl 请求覆盖彼此正文。
